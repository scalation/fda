...
import scalation.calculus.{Poly,PiecewisePoly}

//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePoly` class provides B-Spline basis functions for various orders 'm',
 ...
 */
class B_SplinePoly (ττ: VectorD, mMax: Int = 4)
      extends Error
{
    ...
    val splines = makeSplines()
    
    if (mMax < 1 || mMax > 10) flaw ("constructor", "B_SplinePoly order restricted to 1 thru 10")

    if (DEBUG) {
        println (s"B_SplinePoly (ττ = $ττ, mMax = $mMax)")
    ...
    */
    def bb(m:Int)(j:Int,t:Double): Double = splines(m-1)(j)(t)
    ...
    /**
     */
    def makeSplines() : Array[Array[PiecewisePoly]] =
    {
            val knot = τ
            val ends = for ( i <- 0 until τ.dim-1 ) yield if (i<τ.dim-2) (true,false) else (true,true)
	    var splines = Array.ofDim[Array[PiecewisePoly]](mMax)
            //var splines = Array.ofDim[PiecewisePoly](τ.dim-1)
	    splines(0) = Array.ofDim[PiecewisePoly](τ.dim-1)
            for ( i <- 0 until τ.dim-1 ) {
                val poly = for ( j <- 0 until τ.dim-1 ) yield if ( j < mMax-1 || j >= l + mMax - 1 || j!=i ) Poly(0) else Poly(1)
                splines(0)(i) = PiecewisePoly(τ,ends.toVector,poly.toVector)
            } // for
	    if (DEBUG) println("Building up the order > 1 splines...")
            for ( k <- 2 to mMax ){									// chose your oder
	    	splines(k-1) = Array.ofDim[PiecewisePoly](splines(k-2).length-1)
	    	if (DEBUG) println(s"Splines of order $k")
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest2` object is used to test the `B_SplinePoly` class.
 *  It tests the B-Spline functions using the general recurrence.
 *  > run-main scalation.analytics.fda.B_SplinePolyTest2
 */
object B_SplinePolyTest2 extends App
{
    import scalation.plot.Plot

    val mM = 4                                               // maximum order to test
    val τ  = VectorD (0.0, 20.0, 40.0, 60.0, 80.0, 100.0)    // knot time-points
    val bs = new B_SplinePoly (τ, mM)                            // B-Spline generator
...
} // B_SplinePolyTest2 object

//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest3` object is used to test the `B_SplinePoly` class.
 *  It tests the B-Spline functions using the general recurrence and plots
 *  several basis functions using `PlotM`.
 *  > run-main scalation.analytics.fda.B_SplinePolyTest3
 */
object B_SplinePolyTest3 extends App
{
    import scalation.linalgebra.MatrixD
    import scalation.plot.PlotM
...
} // B_SplinePolyTest3 object

//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest4` object is used to test the `B_SplinePoly` class.
  *  It tests the B-Spline functions using the general recurrence.
  *  > run-main scalation.analytics.fda.B_SplinePolyTest4
  */
object B_SplinePolyTest4 extends App
{
    val mM = 4                                        // maximum order to test
...
} // B_SplinePolyTest4 object

//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest5` object is used to test the `B_SplinePoly` class. Here, we
  * compute the penalty matrix for a ridge regression.
  * > run-main scalation.analytics.fda.B_SplinePolyTest5
  */
object B_SplinePolyTest5 extends App
{
    val mM = 4                                         // maximum order to test
    val n  = 100                                       // number of time points
    val τ  = VectorD (0.0, 0.5 * (n-1), (n-1)) / n     // knot vector (unaugmented)
    ...
    println (s" Σ = $Σ")
    println (s"λΣ = ${Σ * λ}")
} // B_SplinePolyTest5 object
 
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest6` object is used to test the `B_SplinePoly` class. Here, we
  * smooth functional data manually with a roughness penalty.
  *@ see Ramsay et al. page 86
  * > run-main scalation.analytics.fda.B_SplinePolyTest5
  */
object B_SplinePolyTest6 extends App
{
...
} // B_SplinePolyTest6 object


//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `B_SplinePolyTest4` object is used to test the `B_SplinePoly` class.
  *  It tests the B-Spline functions using the general recurrence.
  *  > run-main scalation.analytics.fda.B_SplinePolyTest7
  */
object B_SplinePolyTest7 extends App
{
    import scalation.plot.FPlot
    val mM = 4                                        // maximum order to test
    val n  = 4                                      // number of time points
    val τ  = VectorD (0.0,1.0,2.0,3.0)		       // knot vector (unaugmented)
    val bs = new B_SplinePoly (τ, mM)                     // B-Spline generator
    val fs = bs.splines(mM-1).toSeq.map( f => ( (x:Double) => f(x) ) )
    val dfs = bs.splines(mM-1).toSeq.map( f => ( (x:Double) => f.derivative(x) ) ) 
    val plot  = new FPlot(0.0 until 3.0 by 0.01, fs, lines = true )
    val plot2 = new FPlot(0.0 until 3.0 by 0.01, dfs, lines = true )
    val t  = VectorD.range (0, 300)/100                  // time-points for plotting
    for (m <- 1 to mM) bs.plot (m)(t)                 // plot
