...
object GapStatistic
{
     ...
     */
    def KMeansPlusPlus (x: MatrixD, kMax: Int, algo: Algorithm = HARTIGAN,
    		       	b: Int = 10, useSVD: Boolean = true, plot: Boolean = false): (KMeansPlusPlusClusterer, Array [Int], Int) =
    {
...
//        var opcls: Array [Int] = null
	    ...
	    for (l <- 0 until b){  						
	    	...
	    	val (rcl, rcls)  = KMeansPlusPlusClusterer.run (ref, k+1, algo) // compute and store the log of the withinSSE for each ref distribution
	    ...
	    sdks(k) = Math.pow(sdks(k),1/2)					// the final value for the sd of the ref dist's
            ...
            val (acl, acls)  = KMeansPlusPlusClusterer.run (  x, k+1, algo)	// calculate the actuale KMeansPlusPlus Clustering for the data for k many clusters 
