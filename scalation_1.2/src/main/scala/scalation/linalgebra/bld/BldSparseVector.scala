 ...
 *  > run-main scalation.linalgebra.bld.BldSparseVector
 */
...
//    extends Traversable [$BASE] with PartiallyOrdered [Sparse$VECTOR] with Vec with Error with Serializable
{
     ...
     */
    def this (iv: Tuple2 [Int, $BASE], dm: Int) { this (dm); this(iv._1) = iv._2 }
     ...
     */
    def + (s: Tuple2 [Int, $BASE]): Sparse$VECTOR =
    {
        if (s._1 =~ $ZERO) this
        else {
            val c = new Sparse$VECTOR (this)
            c(s._1) += s._2
            c
        } // if    
    } // +
