 ...
 */
class TightClustering (x: MatrixD, k: Int, s: Int = 0, primary: Boolean = true)
      extends Clusterer with Error
{
    ...
    */
	...
	val cl = new TightClustering(uniDMatrix, k, s, primary)
	cl.w()
    }
...
object TightClusteringTest extends App
{
    ...
    for (s <- 0 to 4) {                         // test with different random streams
        banner ("TightClustering for stream s = " + s)
        val cl = new TightClustering (v, 3, s)                 
